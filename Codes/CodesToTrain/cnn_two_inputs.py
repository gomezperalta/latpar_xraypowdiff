...
def model_branch(input_shape = (1,1), output_dims = 1, conv_arch = list(), conv_act = list(),
    ...
    for item, layer in enumerate(conv_arch):
        ...
        if item == 0:
            ...
            if layer == 'Conv':
                ...
                x = Layers.AveragePooling1D(pool_size=pool_size, 
                                     stride=pool_stride,
    ...
    x = Layers.BatchNormalization(name='BNGAP' + str(serie).zfill(3))(x)
    #'''
...
def main_function(control_file = 'txt-file_name', output_dims = 1, test_val_frac=0.15):
    ...
    df = pd.read_csv('fullColl/dfset_macro.csv') #This is a support file to take interplanar distances
    minpeaks = 6   #Loading file with number of peaks higher than x-intensity
    peaks = pd.read_csv('fullColl/db_peaks.csv') #File containing the number of peaks with an intensity >= 5% with 50 nm
    cond0 = peaks[peaks['numpeaks'] >= minpeaks].index  #Indices that satisfy condition of minpeaks.    
 
    y = np.load('fullColl/yts2_sxv.npy') #This is first called to get the indices of the samples to work with.
    y = y[cond0]
    ...
    df_temp = pd.read_csv('fullColl/dfset_0050.csv')
    df_temp = df_temp.take(cond0).reset_index(drop=True)
    df_temp = df_temp.take(cond).reset_index(drop=True)
    df = pd.concat((df,df_temp), ignore_index=True)

    df_temp = pd.read_csv('fullColl/dfset_0100.csv')
    df_temp = df_temp.take(cond0).reset_index(drop=True)
    df_temp = df_temp.take(cond).reset_index(drop=True)
    df = pd.concat((df,df_temp), ignore_index=True)

    df_temp = pd.read_csv('fullColl/dfset_0250.csv')
    df_temp = df_temp.take(cond0).reset_index(drop=True)
    df_temp = df_temp.take(cond).reset_index(drop=True) 
    df = pd.concat((df,df_temp), ignore_index=True)
    
    #Joining input_data
    x = np.load('fullColl/xset_macro.npy')
    x = x[cond0,:,:]
    x = x[cond,:,:]

    x_temp = np.load('fullColl/xset_0050.npy')
    x_temp = x_temp[cond0,:,:]
    x_temp = x_temp[cond,:,:]
    x = np.concatenate((x,x_temp),axis=0)

    x_temp = np.load('fullColl/xset_0100.npy')
    x_temp = x_temp[cond0,:,:]
    x_temp = x_temp[cond,:,:] 
    x = np.concatenate((x,x_temp),axis=0)

    x_temp = np.load('fullColl/xset_0250.npy')
    x_temp = x_temp[cond0,:,:] 
    x_temp = x_temp[cond,:,:] 
    x = np.concatenate((x,x_temp),axis=0)
    x_temp = ''
    ...
    x2 = np.load('fullColl/binformula.npy')
    x2 = x2[cond0,:,:]
    x2 = x2[cond,:,:]
